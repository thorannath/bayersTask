...
export const fetchLabels = () => async (dispatch) => {
    ...
    try {
        ...
        const res = (await axios.get(baseURL + "/patientfinder/labels", { params })).data;
        if (res.success) {
            let medicalConditions = res.data.filter(data => data.label_type === constants.labelTypes.MEDICAL_CONDITION);
            let treatments = res.data.filter(data => data.label_type === constants.labelTypes.TREATMENT);
